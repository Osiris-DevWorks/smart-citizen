...
- **Installationsort überprüfen** — prüft, ob Smart Citizen auf die Star-Citizen-Kopie zeigt, die du tatsächlich spielst. Wenn auf deinem PC mehrere Installationen liegen (ein verschobener Bibliotheksordner, eine zweite Festplatte, Reste einer Neuinstallation), kann die App in genau die schreiben, die der RSI-Launcher nie startet: Das Anwenden meldet Erfolg, im Spiel ändert sich aber nichts. Diese Schaltfläche findet jede erreichbare Installation und nennt dir die, die der Launcher wirklich verwendet — sie liest dazu das Launcher-Protokoll, statt anhand von Ordnernamen zu raten. Jede gefundene Installation wird mit ihren Belegen aufgelistet, und **Diese Installation verwenden** richtet Smart Citizen mit einem Klick darauf aus. Liegt deine Installation an einem ungewöhnlichen Ort, sucht **Alle Laufwerke durchsuchen** langsamer, dafür gründlicher.
...
- **Anwenden meldet Erfolg, aber im Spiel ändert sich nichts** — Möglicherweise hast du mehrere Star-Citizen-Installationen, und Smart Citizen zeigt auf die, die du nicht startest. Klicke im **Konfiguration-Tab** auf **Installationsort überprüfen**: Es nennt die Installation, die der RSI-Launcher tatsächlich verwendet, und richtet die App mit einem Klick neu aus.
