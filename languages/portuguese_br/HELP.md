...
- **Escanear logs automaticamente ao iniciar** executa a mesma varredura automaticamente toda vez que o Smart Citizen inicia, mantendo sua lista de adquiridos atualizada sem um clique manual. Desativado por padrão. O resultado é informado discretamente pela barra de status em vez de um popup, então uma inicialização que não encontra nada novo permanece silenciosa.
- **Mostrar popup para novos blueprints encontrados pela varredura automática** traz de volta o popup de resumo normal (com a lista expansível do que foi adicionado) quando a varredura automática de inicialização encontra algo novo, em vez de apenas a mensagem na barra de status. Desativado por padrão. Só tem efeito quando a varredura automática também está ativada, e apenas quando ela realmente encontra algo novo.
