...
- **Analizza automaticamente i log all'avvio** esegue la stessa scansione automaticamente a ogni avvio di Smart Citizen, così la tua collezione posseduta resta aggiornata senza un clic manuale. Disattivato per impostazione predefinita. Il risultato viene segnalato silenziosamente nella barra di stato invece che con un popup, quindi un avvio che non trova nulla di nuovo resta silenzioso.
- **Mostra popup per i nuovi blueprint trovati dalla scansione automatica** riporta il normale popup di riepilogo (con l'elenco espandibile di ciò che è stato aggiunto) quando la scansione automatica all'avvio trova qualcosa di nuovo, invece del solo messaggio nella barra di stato. Disattivato per impostazione predefinita. Ha effetto solo se anche la scansione automatica è attiva, e solo quando trova davvero qualcosa di nuovo.
