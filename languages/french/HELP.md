...
- **Rechercher automatiquement dans les journaux au démarrage** relance la même recherche automatiquement à chaque démarrage de Smart Citizen, pour que votre collection reste à jour sans clic manuel. Désactivé par défaut. Le résultat s'affiche discrètement dans la barre d'état plutôt que dans une fenêtre, donc un démarrage qui ne trouve rien de nouveau reste silencieux.
- **Afficher une fenêtre pour les nouveaux plans trouvés par l'analyse automatique** réactive la fenêtre de résumé habituelle (avec la liste déroulante de ce qui a été ajouté) quand la recherche automatique au démarrage trouve quelque chose de nouveau, au lieu du simple message dans la barre d'état. Désactivé par défaut. Ne s'applique que si la recherche automatique est elle aussi activée, et seulement quand elle trouve réellement quelque chose de nouveau.
