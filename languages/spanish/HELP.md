...
- **Escanear registros automáticamente al iniciar** ejecuta el mismo escaneo automáticamente cada vez que Smart Citizen se inicia, para que tu colección se mantenga al día sin un clic manual. Desactivado por defecto. El resultado se informa discretamente en la barra de estado en lugar de una ventana emergente, así que un inicio que no encuentra nada nuevo permanece silencioso.
- **Mostrar ventana emergente para nuevos blueprints encontrados por el análisis automático** vuelve a mostrar la ventana de resumen normal (con la lista desplegable de lo que se añadió) cuando el análisis automático al iniciar encuentra algo nuevo, en lugar de solo el mensaje en la barra de estado. Desactivado por defecto. Solo importa cuando el análisis automático también está activado, y solo cuando realmente encuentra algo nuevo.
