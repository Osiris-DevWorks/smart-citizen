...
- `test_<language>_activation.py` (spanish, japanese, chinese, italian, german, turkish): per-language activation locks: the real `ui.json` is `{ht, at}`-shaped, covers the full English key set (no extras outside `tutorial.*`), keeps every `{placeholder}`, translates the 19-step tour, shows in the selector, maps to the right SC language id, and has a `sources.json` URL plus the four translated docs. `test_turkish_activation.py` (#404) additionally locks the **borrowed** `polish_(poland)` slot (not `turkish_(turkey)`, which the game does not accept; not `russian_(russia)`, which the game does not recognise either; not Dymerz's `german_(germany)`, which our German owns) and that `ensure_user_cfg_language` writes that slot to `user.cfg`.
