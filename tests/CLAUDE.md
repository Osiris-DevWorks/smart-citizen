...
- `test_install_scanner.py`: 2.4/#385 "find every Star Citizen install" core (`src/utils/install_scanner.py`, deliberately Qt-free and settings-free — every input is passed in, unit-tests against a temp directory tree). `TestVerdicts` covers `ScanReport.verdict`'s full state table (single/leftover/match/mismatch/unconfigured/unknown-active, including the leftover-blind-spot cases a "same install" or "no configured root" check would otherwise mask). `TestUnusedReporting`, `TestDeepScan` (depth bound, prune-on-match, skip-list, cancel), and `TestSharedContracts` (channel names track `AppSettings.AVAILABLE_CHANNELS`, the apply-stamp marker matches `main_window`'s writer, version-string sort ordering) round it out. `TestProgressLabelElision` covers `workers.elide_middle`.
- `test_duplicate_install_badges.py`: 2.4/#385 `DuplicateInstallDialog._badges()` / use-button tooltip color-consistency regressions — the badge shown on an install card must never contradict the banner's own verdict color for the same state (an unproven verdict like "no launcher log" must stay warn, not render a configured-install badge red), and a leftover install's tooltip must win over "already configured" when both are true, since the leftover fact is the more urgent one.
- `test_install_scan_finished_stale_worker.py`: 2.4/#385 `ConfigTab` install-scan lifecycle guards — three real concurrency bugs the deep scan's Cancel button and lack of a re-entry guard made reachable (re-scan-before-cancelled-worker's-`finished`-arrives calling `.wait()` on the wrong, still-running worker; a late cancelled-worker `finished` reopening a dialog the user already dismissed; a second click on "Scan all drives" while one is already running orphaning the first scan's progress dialog). All driven on a lightweight stub `self`, no QApplication, no real `QThread`.
