...
- `test_settings_profile_defaults.py`: #383. A setting is only stored once changed, so anything still at its default had no key to enumerate and never reached the backup — meaning an import could not restore a preference *to* its default (rename a Mission Label, import an older backup, the rename survived). Covers `AppSettings.profile_default_values()` materialising into `export_all_values`, stored values still winning over defaults, the issue's own export→change→import scenario, and the `PROFILE_DEFAULT_EXCLUDE_KEYS` carve-outs (install paths, `owned_items`, the log watermark, test-plan keys) staying out so an import restores rather than destroys. `TestNoDrift` is the important half: it replays every no-arg `AppSettings.get_*` against a recording backend and fails if a defaulted key is neither materialised nor deliberately excluded, so a newly added setting can't silently reintroduce the same bug — verified to fail when a key is removed from the map.
