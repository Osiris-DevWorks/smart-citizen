...
- **Show popup for new blueprints found by auto-scan** brings back the normal summary popup (with the expandable list of what was added) when the startup auto-scan finds something new, instead of just the status bar message. Off by default. Only matters when auto-scan is also on, and only when it actually finds something new.
